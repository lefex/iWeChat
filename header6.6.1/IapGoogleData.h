//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <MMCommon/WXPBGeneratedMessage.h>

@class NSString;

@interface IapGoogleData : WXPBGeneratedMessage
{
}

+ (void)initialize;

// Remaining properties
@property(retain, nonatomic) NSString *currencyType; // @dynamic currencyType;
@property(retain, nonatomic) NSString *dataSign; // @dynamic dataSign;
@property(retain, nonatomic) NSString *payload; // @dynamic payload;
@property(retain, nonatomic) NSString *price; // @dynamic price;
@property(retain, nonatomic) NSString *productId; // @dynamic productId;
@property(retain, nonatomic) NSString *purchaseData; // @dynamic purchaseData;
@property(retain, nonatomic) NSString *sessionData; // @dynamic sessionData;

@end

